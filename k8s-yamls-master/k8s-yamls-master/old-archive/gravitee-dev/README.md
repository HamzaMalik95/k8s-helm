...
kubectl config set-context gravitee --namespace=gravitee-dev --cluster=cadev --user=mdeckert && kubectl config use-context gravitee


# Generate yaml for gravitee apim 3.x via helm3

## Save to a file
helm template --name-template=gio graviteeio/apim3 --kube-context gravitee -f /mnt/d/git/k8s/k8s-yamls/live/cadev/gravitee/apim/values.yaml > /mnt/d/git/k8s/k8s-yamls/live/cadev/gravitee/apim/gravitee.yaml
...
kubectl apply -f /mnt/d/git/k8s/k8s-yamls/live/cadev/gravitee/apim/1-deploy-redis.yaml
kubectl apply -f /mnt/d/git/k8s/k8s-yamls/live/cadev/gravitee/apim/2-cm-redis.yaml

# Add cert (if not already)
kubectl apply -f /mnt/d/git/k8s/k8s-yamls/live/cadev/gravitee/apim/2-secret-medimpact-certs.yaml

# Add additional services within the namespace for internal component use
kubectl apply -f /mnt/d/git/k8s/k8s-yamls/live/cadev/gravitee/apim/3-svc-api-dev.yaml
kubectl apply -f /mnt/d/git/k8s/k8s-yamls/live/cadev/gravitee/apim/4-svc-api-qa.yaml

# Add ingresses for gateway itself
kubectl apply -f /mnt/d/git/k8s/k8s-yamls/live/cadev/gravitee/apim/5-ing-gateway.yaml

# Install APIM
kubectl apply -f /mnt/d/git/k8s/k8s-yamls/live/cadev/gravitee/apim/gravitee.yaml
